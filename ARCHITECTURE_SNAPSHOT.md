...
- `src/history.rs`：检测历史快照持久化（`dist/history/snapshots.json`）
...
- TLS：双模式（快速 ClientHello 预检 / 标准 rustls 握手）+ 协议条件约束
- UDP：通过 / 部分 / 失败 / 跳过
- TTFB：
...
- 导出筛选：协议类型 / 最小安全评分 / 最大 TCP 均延迟 / 最佳节点数量
- 历史快照：每次检测完成自动记录，可在窗口中查看通过率、失败率与上次对比趋势
- 本地导入：目录扫描带深度、文件数与单文件大小保护，降低误选大目录造成的长时间等待风险
