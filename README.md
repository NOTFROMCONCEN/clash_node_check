...
- 订阅级汇总与 GUI
  ...
  - 支持导出二次筛选（协议类型 / 最小安全评分 / 最大 TCP 均延迟）
  - 支持检测历史快照自动记录与历史窗口查看
...
- 检测：启用/关闭 TLS 检测、切换 TLS 模式（快速预检/标准握手）、切换稳定性窗口、重置检测参数
- 视图：切换精简/完整表格模式、打开选中节点详情、打开历史快照窗口
- 说明：指标说明、批量导入导出说明、关于
...
- 暂未接入吞吐测试、出口 IP / ASN / 地区核验、IPv4 / IPv6 能力检测、业务场景可达性。
